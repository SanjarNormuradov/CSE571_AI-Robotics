...
class ArmEnvironment:
    def __init__(self, urdf_file, start_pos, goal_pos, obstacles, vis_plan=False, num_discretize=100):
        ...
        self.set_ee_link()
        self.joint_info = self.get_revolute_joint_info() # ((J1_index, J1_lower_limit, J1_upper_limit), (J2_index, J2_lower_limit, J2_upper_limit),...)
        ...
        # self.theta_discretized.shape = [[0...self.num_discretize-1], [0...self.num_discretize-1], ]
    ...
    def visualize_plan(self, plan=None, tree=None, visited=None, plot=False):
        """
        ...
        if plan is not None:
            plan = plan.T
            print(f"plan.shape: {plan.T.shape}")
        ...
        art1 = self.fig.text(s=f"Cost = {tree.path_cost:8.4f}\nTime Cost = {tree.time_cost:8.4f}\nRandom Seed = {tree.rand_seed}",
                                 x=.36, y=.98, ha='left', va='top', fontsize='x-large')
        art2 = self.fig.text(s=f"Bias = {tree.bias}\nEta = {tree.eta}\n#Obstacles = {len(self.obstacles)}\n#Discretization = {self.num_discretize}",
                                 x=.65, y=.98, ha='left', va='top', fontsize='x-large')
        
        if plot:
            imagename = f"{tree.output_directory}/Path_2dofRobotArm_bias{tree.bias}_eta{tree.eta}_obs{len(self.obstacles)}_nd{self.num_discretize}_rs{tree.rand_seed}"
    ...
    def follow_path(self, path):
        ...
        # print(f"Cost: {sum(cost):8.4f}\n")
        # print("Press Enter to start following the path...")
        # input()  
        # print(path)
        for indices in path:
            ...
            count += 1
        # print("Press Enter to close simulation...")
        # input()
        return True
    ...
    def set_robot_joint(self, joint_indices, joint_angle_or_state):
        joint_angles = joint_angle_or_state
        if isinstance(joint_angle_or_state, np.ndarray): # joint_state
            joint_angles = [self.theta_discretized[i][joint_angle_or_state[i,0]] for i in range(len(self.joint_info))]
        elif isinstance(joint_angle_or_state, list): # joint_angles (used in self.collision_free_ik() function)
            joint_angles = joint_angle_or_state
        # Set the robot's joint angles
        for k in range(len(joint_angles)):
            p.resetJointState(self.ROBOT_ID, joint_indices[k], joint_angles[k])
    ...
    def get_revolute_joint_info(self):
        ...
        return joint_info # ((joint1_index, joint1_lower_limit, joint1_upper_limit), (joint2_index, joint2_lower_limit, joint2_upper_limit),...)
