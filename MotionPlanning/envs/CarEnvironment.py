...
class CarEnvironment(object):
    ...
    def simulate_car(self, x_near, x_rand, linear_vel, steer_angle):
        ...
        # Simulate forward from x_near using the controls (linear_vel, steer_angle) to generate the rollout
        x = x_near
        rollout = [x]
    ...
    def angular_difference(self, start_config, end_config):
        ...
        ang_diff = th1-th2
        ang_diff = ((ang_diff + np.pi) % (2*np.pi)) - np.pi # Wrap the angle to the range [-np.pi, np.pi) 
