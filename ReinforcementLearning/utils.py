...
def rollout(
    ...
    while path_length < episode_length:
        ...
        # truncated - reached timelimit, go out of physical bounds -> end the episode prematurely
        # terminated - terminal state is reached or not
        # reward_info = {'reward_dist', 'reward_ctrl'} -> reward = reward_dist + reward_ctrl
        next_obs, reward, terminated, truncated, reward_info = env.step(copy.deepcopy(action))
...
def get_expert_data(file_path):
    ...
    print('Imported Expert DSata successfully')
    ...
    # print([expert_data[0][key].shape for key in expert_data[0].keys()])
    return expert_data
