...
class Field:
    ...
    def V(self, x, u):
        ...
        prev_x, prev_y, prev_theta = x.ravel()
        rot1, trans, rot2 = u.ravel()
        Vmat = np.array([-trans * np.sin(prev_theta + rot1), np.cos(prev_theta + rot1), 0, trans * np.cos(prev_theta + rot1), np.sin(prev_theta + rot1), 0, 1, 0, 1])       
        return Vmat.reshape((3, 3)) 
    ...
    def sample_noisy_observation(self, x, marker_id, beta=None):
        ...
        if self.use_learned_observation_model:
            ...
            z = np.random.multivariate_normal(z.ravel(), beta).reshape((-1, 1))
            return z
